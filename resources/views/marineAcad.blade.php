...
<div class="container mt-2">
    ...
    <table id="passengers-table" class="table table-bordered table-striped">
        ...
        <tbody>
            @foreach($macademys as $macademy)
            <tr>
                <td>{{ $macademy->id }}</td>
                <td>{{ $macademy->year }}</td>
                <td>{{ $macademy->number_of_passenger_carried }}</td>
                <td>{{ $macademy->number_vehicle_in_fleet }}</td>
                <td>{{ $macademy->revenue_from_operation }}</td>
                <td>{{ $macademy->number_of_employees }}</td>
                <td>{{ $macademy->annual_cost_of_vehicle_maintenance }}</td>
                <td>{{ $macademy->number_of_accidents }}</td>
                <td>
                    <button class="btn btn-warning btn-sm edit-record" data-id="{{ $macademy->id }}" data-toggle="modal" data-target="#editModal" data-record="{{ $macademy }}">Edit</button>
                    <button class="btn btn-danger btn-sm delete-record" data-id="{{ $macademy->id }}" data-toggle="modal" data-target="#deleteModal" data-record="{{ $macademy }}">Delete</button>
                    <button class="btn btn-info btn-sm view-chart" data-id="{{ $macademy->id }}" data-toggle="modal" data-target="#chartModal" data-record="{{ $macademy }}">View Chart</button> 
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form id="addForm" action="{{ route('maritimeAcademy.store') }}" method="POST">
            @csrf
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form id="editForm" action="{{ route('maritimeAcademy.update', '') }}" method="POST">
            @csrf
            @method('PUT')
            <div class="modal-content">
                <div class="modal-header">
...
<div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form id="deleteForm" action="{{ route('maritimeAcademy.destroy', '') }}" method="POST">
            @csrf
            @method('DELETE')
            <div class="modal-content">
                <div class="modal-header">
...
<script>
    $(document).ready(function() {
        ...
        $('#add-marineAcad').on('click', function() {
            $('#addModal').modal('show');
        });

        // Handle Edit button click
        $('.edit-marineAcad').on('click', function() {
            var passenger = $(this).data('marineAcad');
            ...
            $('#editForm').attr('action', '/marineAcad/' + macademy.id);
            $('#editModal').modal('show');
        });

        // Handle Delete button click
        $('.delete-macademy').on('click', function() {
            var macademy = $(this).data('macademy');
            $('#deleteForm').attr('action', '/marineAcad/' + macademy.id);
