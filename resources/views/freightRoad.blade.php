    ...
    <table id="freights-table" class="table table-bordered table-striped">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $freight->id }}</td>
                <td>{{ $freight->year }}</td>
                <td>{{ $freight->number_of_tonnes_carried }}</td>
                <td>{{ $freight->number_vehicle_in_fleet }}</td>
                <td>{{ $freight->revenue_from_operation }}</td>
                <td>{{ $freight->number_of_employees }}</td>
                <td>{{ $freight->annual_cost_of_vehicle_maintenance }}</td>
                <td>{{ $freight->number_of_accidents }}</td>
                <td>
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form id="addForm" action="{{ route('freightRoadTransport.store') }}" method="POST">
            @csrf
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="addNumberOfTonnes">Number Of
                          Tonnes</label>
                        <input type="number" class="form-control" id="addNumberOfTonnes" name="number_of_tonnes_carried" required>
                    </div>
                    <div class="form-group">
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form id="editForm" action="{{ route('freightRoadTransport.update', '') }}" method="POST">
            @csrf
            @method('PUT')
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="editNumberOfTonnes">Tonnes</label>
...
<div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form id="deleteForm" action="{{ route('freightRoadTransport.destroy', '') }}" method="POST">
            @csrf
            @method('DELETE')
            <div class="modal-content">
                <div class="modal-header">
...
<script>
    $(document).ready(function() {
        ...
        $('#add-freight').on('click', function() {
            $('#addModal').modal('show');
        });

        // Handle Edit button click
        $('.edit-freight').on('click', function() {
            var passenger = $(this).data('freight');
            $('#editId').val(freight.id);
            $('#editYear').val(freight.year);
            $('#editNumberOfTonnes').val(freight.number_of_tonnes_carried);
            $('#editVehicles').val(freight.number_of_vehicle_in_fleet);
            $('#editRevenue').val(freight.revenue_from_operation);
            $('#editEmployees').val(freight.number_of_employees);
            $('#editCost').val(freight.annual_cost_of_vehicle_maintenance);
            $('#editAccidents').val(freight.number_of_accidents);
            $('#editForm').attr('action', '/freightRoad/' + freight.id);
            $('#editModal').modal('show');
        });

        // Handle Delete button click
        $('.delete-freight').on('click', function() {
            var freight = $(this).data('freight');
            $('#deleteForm').attr('action', '/freightRoad/' + freight.id);
