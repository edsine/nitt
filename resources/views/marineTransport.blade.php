...
<div class="container mt-2">
    ...
    <table id="passengers-table" class="table table-bordered table-striped">
        ...
        <tbody>
            @foreach($mtransports as $mtransport)
            <tr>
                <td>{{ $mtransport->id }}</td>
                <td>{{ $mtransport->year }}</td>
                <td>{{ $mtransport->number_of_passenger_carried }}</td>
                <td>{{ $mtransport->number_vehicle_in_fleet }}</td>
                <td>{{ $mtransport->revenue_from_operation }}</td>
                <td>{{ $mtransport->number_of_employees }}</td>
                <td>{{ $mtransport->annual_cost_of_vehicle_maintenance }}</td>
                <td>{{ $mtransport->number_of_accidents }}</td>
                <td>
                    <button class="btn btn-warning btn-sm edit-record" data-id="{{ $mtransport->id }}" data-toggle="modal" data-target="#editModal" data-record="{{ $mtransport }}">Edit</button>
                    <button class="btn btn-danger btn-sm delete-record" data-id="{{ $mtransport->id }}" data-toggle="modal" data-target="#deleteModal" data-record="{{ $mtransport }}">Delete</button>
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form id="addForm" action="{{ route('maritimeTransport.store') }}" method="POST">
            @csrf
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form id="editForm" action="{{ route('maritimeTransport.update', '') }}" method="POST">
            @csrf
            @method('PUT')
            <div class="modal-content">
                <div class="modal-header">
...
<div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <form id="deleteForm" action="{{ route('maritimeTransport.destroy', '') }}" method="POST">
            @csrf
            @method('DELETE')
            <div class="modal-content">
                <div class="modal-header">
