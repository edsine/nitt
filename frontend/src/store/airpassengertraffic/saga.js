...
function* fetchAirPassengerTraffic() {
  try {
    ...
    if (response.success) {
      ...
      yield put(getAirPassengerTrafficFail(Object.values(response?.errors)));
    }
  } catch (error) {
...
function* addAirPassengerTraffic({ payload }) {
  try {
    ...
    if (response?.success) {
      ...
      yield put(addAirPassengerTrafficFail(Object.values(response?.errors)));
    }
  } catch (error) {
...
function* updateAirPassengerTraffic({ payload: { airPassengerTraffic, id } }) {
  try {
    ...
    if (response?.success) {
      ...
      yield put(editAirPassengerTrafficFail(Object.values(response?.errors)));
    }
  } catch (error) {
...
function* removeAirPassengerTraffic({ payload }) {
  try {
    ...
    if (response?.success) {
      ...
      yield put(deleteAirPassengerTrafficFail(Object.values(response?.errors)));
    }
  } catch (error) {
